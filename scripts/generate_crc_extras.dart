...
void main() {
  ...
  buffer.writeln('///');
  buffer.writeln('/// Contains CRC extras for ${sorted.length} messages');
  buffer.writeln('/// from common.xml and ardupilotmega.xml.');
  buffer.writeln('const Map<int, int> mavlinkCrcExtras = {');

  for (final entry in sorted) {
    final msg = entry.value;
    final crc = _computeCrcExtra(msg);
    buffer.writeln('  ${msg.id}: $crc, // ${msg.name}');
  }
  ...
  print('Generated $outPath with ${sorted.length} CRC extras');
}

...
class SerialTransport implements MavlinkTransport {
  ...
  Future<void> connect() async {
    await disconnect();
    // Allow the previous reader isolate to fully terminate before opening
    // a new one. The flutter_libserialport reader isolate can hold a stale
    // dylib path reference that corrupts on immediate re-creation.
  ...
  Future<void> disconnect() async {
    // Cancel subscription first to stop processing data.
    await _readSub?.cancel();
    _readSub = null;

    // Close the reader — this signals the background isolate to shut down.
    // Must happen before closing the port (reader references port handle).
    try {
      _reader?.close();
    } catch (_) {
      // Reader may already be closed or in a bad state.
    }
    _reader = null;
  ...
  Future<void> send(Uint8List data) async {
    ...
    try {
      ...
      // Device was unplugged or became unavailable (errno 6 = ENXIO).
      // Signal the error via stateStream; caller should not crash.
      _setState(TransportState.error);
    }
  }

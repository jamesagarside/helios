...
class ConnectionController extends StateNotifier<ConnectionStatus> {
  ...
  /// this controller's Riverpod providers and services. The router owns the
  /// "which sinks fire" decision; these closures own the side effects.
  MavlinkRouterSinks _buildRouterSinks(Set<int> knownSystems) {
    return MavlinkRouterSinks(
      activeVehicleId: () => _ref.read(activeVehicleIdProvider),
      handleVehicleMessage: (msg) =>
          _ref.read(vehicleStateProvider.notifier).handleMessage(msg),
      isRecording: () => _ref.read(telemetryStoreProvider).isRecording,
      bufferTelemetry: (msg) => _ref.read(telemetryStoreProvider).buffer(msg),
      addAlert: (entry) => _ref.read(alertHistoryProvider.notifier).add(entry),
      updateAdsb: (msg) {
        _ref.read(adsbProvider.notifier).update(msg);
        _ref.read(adsbProvider.notifier).pruneStale();
      },
      inspectorActive: () => _ref.read(inspectorActiveProvider),
      addInspectorPacket: (entry) =>
          _ref.read(mavlinkInspectorProvider.notifier).addPacket(entry),
      knownVehicleIds: () => knownSystems,
      registerVehicle: (systemId) {
        knownSystems.add(systemId);
        final registry =
            Map<int, VehicleState>.from(_ref.read(vehicleRegistryProvider));
        registry[systemId] = const VehicleState();
        _ref.read(vehicleRegistryProvider.notifier).state = registry;

        if (_ref.read(activeVehicleIdProvider) == 0) {
          _ref.read(activeVehicleIdProvider.notifier).state = systemId;
        }
      },
      syncVehicleToRegistry: (systemId) {
        final current = _ref.read(vehicleStateProvider);
        if (systemId == current.systemId) {
          final registry =
              Map<int, VehicleState>.from(_ref.read(vehicleRegistryProvider));
          registry[systemId] = current;
          _ref.read(vehicleRegistryProvider.notifier).state = registry;
        }
      },
      onFirstHeartbeat: (systemId, componentId) {
        ...
        _requestFirmwareVersion(systemId, componentId);

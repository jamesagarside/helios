...
import 'telemetry_tile_config.dart';
...
/// Extra telemetry readouts that can be shown on the PFD overlay.
enum PfdExtra {
  groundspeed('GS', 'm/s'),
  battery('BATT', 'V'),
  throttle('THR', '%'),
  ...
  const PfdExtra(this.label, this.unit);
  final String label;
  final String unit;
}
...
class LayoutProfile {
  const LayoutProfile({
    ...
    this.showServoPanel = false,
    this.showRcPanel = false,
    this.pfdExtras = const {PfdExtra.groundspeed, PfdExtra.battery},
  });
  ...
  factory LayoutProfile.fromJson(Map<String, dynamic> json) {
    return LayoutProfile(
      ...
      telemetryTiles: (json['telemetryTiles'] as List<dynamic>?)
          ?.map((e) =>
              TelemetryTileConfig.fromJson(e as Map<String, dynamic>))
          .toList(),
      showMessageLog: json['showMessageLog'] as bool? ?? false,
      showActionPanel: json['showActionPanel'] as bool? ?? true,
      showServoPanel: json['showServoPanel'] as bool? ?? false,
      showRcPanel: json['showRcPanel'] as bool? ?? false,
      pfdExtras: (json['pfdExtras'] as List<dynamic>?)
              ?.map((e) => PfdExtra.values.firstWhere(
                    (v) => v.name == e,
                    orElse: () => PfdExtra.groundspeed,
                  ))
              .toSet() ??
          {PfdExtra.groundspeed, PfdExtra.battery},
    );
  }
  ...
  /// Custom telemetry tile configuration.
  /// If null, falls back to [TelemetryFieldRegistry.defaultTileIds].
  final List<TelemetryTileConfig>? telemetryTiles;

  /// Whether the message log overlay is visible.
  final bool showMessageLog;

  /// Whether the flight action panel is visible.
  final bool showActionPanel;

  /// Whether the servo output diagnostic panel is visible.
  final bool showServoPanel;

  /// Whether the RC input diagnostic panel is visible.
  final bool showRcPanel;

  /// Extra telemetry readouts shown on the PFD overlay.
  ...
  LayoutProfile copyWith({
    ...
    WidgetConfig? video,
    bool? isDefault,
    List<TelemetryTileConfig>? telemetryTiles,
    bool? showMessageLog,
    bool? showActionPanel,
    bool? showServoPanel,
    bool? showRcPanel,
    Set<PfdExtra>? pfdExtras,
  }) {
    return LayoutProfile(
      name: name ?? this.name,
      ...
      telemetryTiles: telemetryTiles ?? this.telemetryTiles,
      showMessageLog: showMessageLog ?? this.showMessageLog,
      showActionPanel: showActionPanel ?? this.showActionPanel,
      showServoPanel: showServoPanel ?? this.showServoPanel,
      showRcPanel: showRcPanel ?? this.showRcPanel,
      pfdExtras: pfdExtras ?? this.pfdExtras,
    );
  }

  /// Effective tile list: custom tiles if configured, else default set.
  List<TelemetryTileConfig> get effectiveTiles {
    if (telemetryTiles != null && telemetryTiles!.isNotEmpty) {
      return telemetryTiles!;
    }
    return TelemetryFieldRegistry.defaultTileIds
        .map((id) => TelemetryTileConfig(fieldId: id))
        .toList();
  }
  ...
  Map<String, dynamic> toJson() => {
        ...
        'video': video.toJson(),
        'isDefault': isDefault,
        if (telemetryTiles != null)
          'telemetryTiles': telemetryTiles!.map((t) => t.toJson()).toList(),
        'showMessageLog': showMessageLog,
        'showActionPanel': showActionPanel,
        'showServoPanel': showServoPanel,
        'showRcPanel': showRcPanel,
        'pfdExtras': pfdExtras.map((e) => e.name).toList(),
      };

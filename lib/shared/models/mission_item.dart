...
class MissionItem extends Equatable {
  ...
  String get commandLabel => switch (command) {
    ...
    MavCmd.doJump => 'Jump',
    MavCmd.doSetCamTriggDist => 'Camera Trigger',
    MavCmd.doMountControl => 'Gimbal Control',
  ...
  /// Serialise to JSON (for mission template storage).
  ...
  /// Deserialise from JSON (for mission template loading).
  factory MissionItem.fromJson(Map<String, dynamic> json) {
    return MissionItem(
      ...
      param1: (json['p1'] as num?)?.toDouble() ?? 0.0,
      param2: (json['p2'] as num?)?.toDouble() ?? 0.0,
      param3: (json['p3'] as num?)?.toDouble() ?? 0.0,
      param4: (json['p4'] as num?)?.toDouble() ?? 0.0,
      frame: (json['frame'] as num?)?.toInt() ?? MavFrame.globalRelativeAlt,
      autocontinue: (json['autoContinue'] as bool? ?? true) ? 1 : 0,
    );
  }
